...
public class SearchThread extends Thread {
	...
	private static final int mFastInternalTime = 10* 1000;
	private static final int mNormalInternalTime = 3 * 60 *1000;
	...
	private void searchDevices() {
		try {
			if (mStartComplete) {
				...
				Log.e(TAG, "controlpoint start:" + startRet);
				if (startRet) {
					mStartComplete = true;
				}
			}
	...
	private DeviceChangeListener mDeviceChangeListener = new DeviceChangeListener() {
		...
		public void deviceRemoved(Device dev) {
			Log.e(TAG, "control point remove a device!!!");
			DLNAContainer.getInstance().removeDevice(dev);
		}

		@Override
		public void deviceAdded(Device dev) {
			Log.e(TAG, "control point add a device->" +dev.getFriendlyName());
