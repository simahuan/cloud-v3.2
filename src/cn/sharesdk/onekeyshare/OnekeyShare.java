...
public class OnekeyShare implements PlatformActionListener, Callback {
	...
	public void show(Context context) {
		...
		try {
			platformListFakeActivity = (PlatformListFakeActivity) Class.forName("cn.sharesdk.onekeyshare.theme.PlatformListPage").newInstance();
		} catch (Exception e) {
			e.printStackTrace(System.err);
			return;
		}

		platformListFakeActivity.setDialogMode(dialogMode); //对话框弹出模式
		platformListFakeActivity.setShareParamsMap(shareParamsMap);//分享参数集合
		platformListFakeActivity.setSilent(silent);//一键分享
		platformListFakeActivity.setCustomerLogos(customers);//自定义Logo
		platformListFakeActivity.setBackgroundView(bgView);//背景色
		platformListFakeActivity.setHiddenPlatforms(hiddenPlatforms);//隐藏平台
		platformListFakeActivity.setOnShareButtonClickListener(onShareButtonClickListener);//分享Button点击事件
		platformListFakeActivity.setThemeShareCallback(new ThemeShareCallback() { //设置数据分享回调方法
			@Override
			public void doShare(HashMap<Platform, HashMap<String, Object>> shareData) {
				share(shareData);
			}
		});
//		LogCat.e("shareParamsMap = "+shareParamsMap.containsKey("platform"));
		if (shareParamsMap.containsKey("platform")) {
			String name = String.valueOf(shareParamsMap.get("platform"));
			Platform platform = ShareSDK.getPlatform(name);
...
//				LogCat.e("viewToShare:="+viewToShare);
			...
			if (!started) {
				started = true;
//				if (this == callback) {  
					int resId = getStringRes(context, "sharing"); //分享操作正在后台进行
					if (resId > 0) {
...
//				}
			...
			plat.setPlatformActionListener(callback);//回调控件　完成,成功,失败 三种情况　
...
//		LogCat.e(".....onComplete......");
		if (platform.getName().equalsIgnoreCase("WechatFavorite")) {
			int resId = getStringRes(context, "favorite_completed"); // 收藏成功
			if (resId > 0) {
				showNotification(context.getString(resId));
			}
		} else {
		...
		switch(msg.what) {
			...
			case MSG_ACTION_CCALLBACK: {
				switch (msg.arg1) {
					case 1: {
						// 成功
						int resId = getStringRes(context, "share_completed"); //分享成功
						if (resId > 0) {
							showNotification(context.getString(resId));
						}
					}
					...
					case 2: {
						...
						if ("WechatClientNotExistException".equals(expName)
							...
							int resId = getStringRes(context, "share_failed");//分享失败
							if (resId > 0) {
								showNotification(context.getString(resId));
							}
						}
					...
					case 3: {
						// 取消
						int resId = getStringRes(context, "share_canceled");//分享已取消
						if (resId > 0) {
							showNotification(context.getString(resId));
						}
					}
