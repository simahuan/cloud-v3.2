...
import com.pisen.router.ui.phone.settings.upgrade.AppVersionResult;

/** platform logo list gridview */
/**
 * 弹出平台GridView 九宫格分布图　
 * @author  mahuan
 * @version 1.0 2015年6月30日 下午2:22:50
 */
public class PlatformGridView extends LinearLayout implements
		OnClickListener, Callback {
	private static final int MIN_CLICK_INTERVAL = 1000;//点击间隔时间
	private static final int MSG_PLATFORM_LIST_GOT = 1;//平台数据
	// grids in each line	
	...
	/**
	 * @des initializes the gridview of platforms
	 */
...
//		0页 则返回 
		...
		// if the total number of pages exceeds 1, we set the page indicators, 多标分页
	...
	public void onClick(final View v) {
		...
		final ArrayList<Object> platforms = new ArrayList<Object>(1);
		platforms.add(v.getTag());
//		LogCat.e("onClick...按键被点击.......");
		dialog.show();
		new HttpGetRequest(context).execute(HttpKeys.REFRESH_APP, "", new OnHttpCallBack() {
			@Override
			public void getHttpResult(String result) {
				...
				if (jsonResult == null || jsonResult.isDataNull()) {
					UIHelper.showToast(context, "您的网络不太顺畅哦~");
				} else {
