...
public class SardineImpl implements Sardine {
	...
	//定义中国对应的GMT常量时间
	private static float TIME_OFFSET = 8 * 60 * 60 * 1000;
	...
	public SardineImpl(String username, String password, ProxySelector selector) {
		init(createDefaultClient(selector), username, password);
	}
...
//		 CoreConnectionPNames.CONNECTION_TIMEOUT, 120* 1000);
	...
	/*
	 * public HttpRequestBase getCurrentRequest() { return _currentRequest; }
	 * 
	 * public void abort() { _isAborted = true; _currentRequest.abort(); }
	 * 
	 * public boolean isAborted() { return _isAborted; }
	 */
	...
	public void setCredentials(String username, String password, String domain, String workstation) {
		if (username != null) {
			this.client.getCredentialsProvider().setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT, AuthScope.ANY_REALM, AuthPolicy.NTLM),
					new NTCredentials(username, password, workstation, domain));
			this.client.getCredentialsProvider().setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT, AuthScope.ANY_REALM, AuthPolicy.BASIC),
					new UsernamePasswordCredentials(username, password));
			this.client.getCredentialsProvider().setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT, AuthScope.ANY_REALM, AuthPolicy.DIGEST),
					new UsernamePasswordCredentials(username, password));
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.aflx.sardine.Sardine#disablePreemptiveAuthentication(java.lang
	...
	public List<DavResource> list(String url, int depth) throws IOException {
		...
		for (Response response : responses) {
			// log.warn("list:  " + response.getHref());
			try {
				resources.add(new DavResource(response));
			} catch (URISyntaxException e) {
				log.error(String.format("Ignore resource with invalid URI %s", response.getHref()));
			}
		}
		return resources;
	}
	...
	/**
	 * Creates a {@link de.aflx.sardine.model.Propertyupdate} element containing
	 * all properties to set from setProps and all properties to remove from
	 * removeProps. Note this method will use a
	...
	public List<DavResource> patch(String url, Map<QName, String> setProps, List<QName> removeProps) throws IOException {
		/*
		 * HttpPropPatch entity = new HttpPropPatch(url); // Build WebDAV
		 * <code>PROPPATCH</code> entity. Propertyupdate body = new
		 * Propertyupdate(); // Add properties { Set set = new Set();
		 * body.getRemoveOrSet().add(set); Prop prop = new Prop(); // Returns a
		 * reference to the live list List<Element> any = prop.getAny(); for
		 * (Map.Entry<QName, String> entry : setProps.entrySet()) { Element
		 * element = SardineUtil.createElement(entry.getKey());
		 * element.setTextContent(entry.getValue()); any.add(element); }
		 * set.setProp(prop); } // Remove properties { Remove remove = new
		 * Remove(); body.getRemoveOrSet().add(remove); Prop prop = new Prop();
		 * // Returns a reference to the live list List<Element> any =
		 * prop.getAny(); for (QName entry : removeProps) { Element element =
		 * SardineUtil.createElement(entry); any.add(element); }
		 * remove.setProp(prop); } // entity.setEntity(new
		 * StringEntity(SardineUtil.toXml(body), UTF_8)); // MS multistatus =
		 * this.execute(entity, // new MultiStatusResponseHandler()); //
		 * List<Response> responses = multistatus.getResponse();
		 * List<DavResource> resources = new ArrayList<DavResource>( 1); // for
		 * (Response response : responses) { // try { // resources.add(new
		 * DavResource(response)); // } catch (URISyntaxException e) { //
		 * log.warn(String.format("Ignore resource with invalid URI %s", //
		 * response.getHref().get(0))); // } // } return resources;
		 */
		return null;
	}
	...
	@Override
	public void put(String url, HttpEntity entity) throws IOException {
		this.put(url, entity, (String) null);
	}

	@Override
	public void put(String url, HttpEntity entity, String contentType) throws IOException {
		this.put(url, entity, contentType, true);
	}
	...
	@Override
	public void put(String url, HttpEntity entity,VoidResponseHandler responseHandle) throws IOException{
		Map<String, String> headers = new HashMap<String, String>();
		boolean expectContinue = true;
		if (expectContinue) {
			headers.put(HTTP.EXPECT_DIRECTIVE, HTTP.EXPECT_CONTINUE);
		}
		headers.put(HTTP.DATE_HEADER,getFormatedDateString((int)TIME_OFFSET));
		//headers.put(HTTP.CHARSET_PARAM, "UTF-8");
		this.put(url, entity, headers,responseHandle);
	}

	public void put(String url, HttpEntity entity, Map<String, String> headers,VoidResponseHandler responseHandler) throws IOException {
		...
		try {
			this.execute(put, responseHandler);
	...
	private String getFormatedDateString(int newTime){
		TimeZone timeZone;
		...
		SimpleDateFormat sdf = new SimpleDateFormat("EE, dd MMM yyyy HH:mm:ss 'GMT'",Locale.ENGLISH);
	...
	public void move(String sourceUrl, String destinationUrl) throws IOException {
        ...
        HttpParams params = move.getParams();
        HttpConnectionParams.setConnectionTimeout(params, 15 * 60 * 1000);
        HttpConnectionParams.setSoTimeout(params, 15 * 60 * 1000);
	...
	protected AbstractHttpClient createDefaultClient(ProxySelector selector) {
		...
		// set timeout
//		LogCat.e("httpconnectionParams timeout setup........");
		HttpConnectionParams.setConnectionTimeout(params, 2*5000);
		HttpConnectionParams.setSoTimeout(params, 2*10000);
	...
	protected HttpParams createDefaultHttpParams() {
		...
		HttpConnectionParams.setSocketBufferSize(params, 8192 * 2);
		return params;
	}
