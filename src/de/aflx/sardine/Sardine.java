...
public interface Sardine {
	 ...
	 * @return List of resources for this URI including the parent resource
	 *         itself
	 * @throws IOException
	 ...
	 *            Path to the resource including protocol and hostname
	 * @param depth
	 *            The depth to look at (use 0 for single ressource, 1 for
	 *            directory listing)
	 * @return List of resources for this URI including the parent resource
	 *         itself
	 * @throws IOException
	 ...
	 *            Path to the resource including protocol and hostname
	 * @param addProps
	 *            Properties to add to resource. If a property already exists
	 ...
	 *            then its value is replaced.
	 * @param removeProps
	 *            Properties to remove from resource. Specifying the removal of
	 *            a property that does not exist is not an error.
	 * @return The patched resources from the response
	 * @throws IOException
	 *             I/O error or HTTP response validation failure
	 */
	...
	/**
	 * Uses HTTP <code>GET</code> to download data from a server. The stream
	 * must be closed after reading.
	 * 
	 * @param url
	 *            Path to the resource including protocol and hostname
	...
	/**
	 * Uses HTTP <code>PUT</code> to send data to a server. Repeatable on
	...
	/**
	 * Uses <code>PUT</code> to send data to a server. Not repeatable on
	 * authentication failure.
	 * 
	 * @param url
	 *            Path to the resource including protocol and hostname
	 ...
	 * type header. Repeatable on authentication failure.
	 * 
	 * @param url
	 *            Path to the resource including protocol and hostname
	 * @param data
	 ...
	 *            Enable <code>Expect: continue</code> header for
	...
	/**
	 * Uses <code>PUT</code> to send data to a server with specific headers. Not
	...
	void put(String url, HttpEntity entity) throws IOException;

	//增加Content-Length,Date字段
	void put(String url, HttpEntity entity,VoidResponseHandler responseHandle) throws IOException;

	void put(String url, HttpEntity entity, String contentType) throws IOException;
	...
	/**
	 * Performs a HTTP <code>HEAD</code> request to see if a resource exists or
	 * not.
	 * 
	 * @param url
	 *            Path to the resource including protocol and hostname
	 * @return Anything outside of the 200-299 response code range returns
	 *         false.
	 * @throws IOException
	 *             I/O error or HTTP response validation failure
	 */
	...
	/**
	 * Put an exclusive write lock on this resource. A write lock must prevent a
	 * principal without the lock from successfully executing a PUT, POST,
	 * PROPPATCH, LOCK, UNLOCK, MOVE, DELETE, or MKCOL on the locked resource.
	 * All other current methods, GET in particular, function independently of
	 * the lock.
	 * <p/>
	 ...
	 * @param url
	 *            Path to the resource including protocol and hostname
	 * @return The lock token to unlock this resource. A lock token is a type of
	 *         state token, represented as a URI, which identifies a particular
	 *         lock. A lock token is returned by every successful
	 *         <code>LOCK</code> operation in the lockdiscovery property in the
	...
	/**
	 * Enables HTTP GZIP compression. If enabled, requests originating from
	 * Sardine will include "gzip" as an "Accept-Encoding" header.
	 * <p/>
	...
	/**
	 * Send a <code>Basic</code> authentication header with each request even
	 * before 401 is returned.
	 * 
	 * @param hostname
